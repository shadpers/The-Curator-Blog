...
# Busca todos os arquivos .mp4 no diretório atual
$arquivos = Get-ChildItem -Filter "*.mp4"
$dadosArquivos = @()

foreach ($arquivo in $arquivos) {
    if ($arquivo.BaseName -match '(\d+)(?:\s*Final)?$') {
        $epNum = [int]$matches[1]
...
Write-Host "  [ 1 ] " -ForegroundColor Cyan -NoNewline; Write-Host "ReZERO S2 Castellano Cap XX.mp4"
Write-Host "  [ 2 ] " -ForegroundColor Cyan -NoNewline; Write-Host "ReZERO S2 - S02EXX.mp4"
Write-Host "  [ 3 ] " -ForegroundColor Cyan -NoNewline; Write-Host "ReZero_S2_XX.mp4"
...
$padrao = ""
switch ($opcao) {
    '1' { $padrao = "ReZERO S2 Castellano Cap XX.mp4" }
    '2' { $padrao = "ReZERO S2 - S02EXX.mp4" }
    '3' { $padrao = "ReZero_S2_XX.mp4" }
    '4' {
