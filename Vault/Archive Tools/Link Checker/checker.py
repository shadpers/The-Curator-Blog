    ...
    try:
        ...
        if has_link_info:
            ...
            if link_info_flags & 0x01:  # VolumeIDAndLocalBasePath
                ...
                # Normaliza o caminho para lidar com diferentes encodings
...
class LinkChecker:
    ...
    def save_scans_history(self, scans):
        # Limita a 10 scans mais recentes
        scans = scans[-10:]
    ...
    def check_pixeldrain(self, url):
        try:
            ...
            if is_album:
                ...
                # Se retornou 200, tenta parsear
                try:
                    data = response.json()
                    
                    # Verifica se tem campo 'success' explícito
                    if 'success' in data and not data['success']:
                        return "OFFLINE", 0
                    
                    # Se tem 'id' no retorno, arquivo existe
                    if 'id' in data:
                        return "ONLINE", 1
                    
                    # Fallback: se chegou aqui com status 200, considera online
                    return "ONLINE", 1
                    
                except json.JSONDecodeError:
                    # Se não conseguiu parsear JSON mas retornou 200, tenta HEAD
    ...
    def show_offline_links(self, results):
        """Exibe todos os links que estão offline."""
        RESET   = "\033[0m"
        CYAN    = "\033[96m"
        RED     = "\033[91m"
        WHITE   = "\033[97m"
        ...
        offline_links = [r for r in results if r['status'] == 'OFFLINE']
        
        if not offline_links:
            print(f"\n{CYAN}{BOLD}Nenhum link offline encontrado! 🎉{RESET}\n")
            return

        print(f"\n{RED}{BOLD}{'═' * 100}{RESET}")
        print(f"      {RED}{BOLD}LINKS OFFLINE ({len(offline_links)} encontrados){RESET}      ".center(100))
        print(f"{RED}{BOLD}{'═' * 100}{RESET}\n")

        current_txt = None
        for entry in offline_links:
            if entry['txt_name'] != current_txt:
                current_txt = entry['txt_name']
                print(f"\n{CYAN}{BOLD}[{current_txt}]{RESET}")
                print(f"{GRAY}{'─' * 80}{RESET}")

            print(f"{WHITE}  • {entry['name']}{RESET}")
            print(f"{GRAY}    {entry['url']}{RESET}")
            if entry['prev_count'] != "N/A" and entry['prev_count'] != 0:
                print(f"{GRAY}    Tinha {entry['prev_count']} arquivo(s) em {entry['prev_date']}{RESET}")
            print()
    ...
    def run_scan(self):
        ...
        for lnk in sorted(lnk_files):
            try:
                ...
                print(f"{RED}Erro ao processar {lnk}: {e}{RESET}")
                continue
                
            if not target:
                print(f"{RED}Erro: Não foi possível extrair o caminho de {lnk}{RESET}")
                continue
            
            # Tenta encontrar o arquivo mesmo com problemas de encoding
            if not os.path.exists(target):
                # Tenta na mesma pasta do .lnk
                lnk_dir = os.path.dirname(os.path.abspath(lnk))
                target_basename = os.path.basename(target)
                alternative_path = os.path.join(lnk_dir, target_basename)
                
                if os.path.exists(alternative_path):
                    target = alternative_path
                else:
                    # Tenta procurar arquivo com nome similar (ignora diferenças de encoding)
                    import unicodedata
                    target_normalized = unicodedata.normalize('NFD', target_basename).encode('ascii', 'ignore').decode('ascii')
                    
                    found = False
                    if os.path.exists(lnk_dir):
                        for file in os.listdir(lnk_dir):
                            file_normalized = unicodedata.normalize('NFD', file).encode('ascii', 'ignore').decode('ascii')
                            if file_normalized.lower() == target_normalized.lower():
                                target = os.path.join(lnk_dir, file)
                                found = True
                                break
                    
                    if not found:
                        print(f"{RED}Erro: Arquivo não encontrado{RESET}")
                        print(f"{GRAY}  Esperado: {target}{RESET}")
                        print(f"{GRAY}  Atalho: {lnk}{RESET}")
                        continue
    ...
    def show_history_menu(self):
        scans = self.load_scans_history()
        if not scans:
            print("\033[93mNenhum scan salvo ainda.\033[0m")
            return
        ...
        # Pergunta se quer ver links offline deste scan
        print()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if escolha == "1":
            ...
            print("\033[93mOpção inválida. Tente novamente.\033[0m")