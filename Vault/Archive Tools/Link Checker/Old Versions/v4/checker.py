...
class LinkChecker:
    ...
    def run(self, input_file):
        if not os.path.exists(input_file):
            print("\033[91m" + f"Erro: Arquivo {input_file} não encontrado." + "\033[0m")
            return
        ...
        print(f"\n{CYAN}{BOLD}{'═' * 100}{RESET}")
        print(f"      {CYAN}{BOLD}VERIFICADOR DE LINKS - STATUS ATUAL{RESET}      ".center(100))
        ...
        for i, line in enumerate(lines, 1):
            ...
            if (str(count).isdigit() and str(p_count).isdigit() and 
                count != p_count and p_count != "N/A" and p_count != "Check Manual"):
